...
app.post("/", async (req, res) => {
  ...
  con.query(
    "INSERT INTO `Users`(`id`, `email`, `username`, `password`, `userimage`) VALUES (?,?,?,?,?)",
    data,
    (err, result) => {
      if (err) {
        return res.json(err);
...
app.post("/addreview", async (req, res) => {
  ...
  con.query(
    "INSERT INTO `Reviews`(`id`, `productid`, `userid`, `text`, `mark`) VALUES (?,?,?,?,?)",
    data,
    (err, result) => {
      if (err) {
        return res.json(err);
      ...
      con.query(
        `SELECT p.id as 'productid', u.id as 'userid', r.id as 'reviewid', u.username, u.userimage, r.text, r.mark FROM Reviews r JOIN Products p on r.productid = p.id JOIN Users u on r.userid = u.id`,
        (err, result) => {
          if (err) {
          ...
          return res.json(Object.values(reviewsData));
        }
      );
    }
  );
...
app.get("/products/attributes", async (req, res) => {
  con.query(
    "SELECT pa.product_id, pa.attribute_name, pa.attribute_value FROM Products_Attributes pa",
    (err, result) => {
      return res.send(result);
    }
  );
...
app.get("/products/characteristics", async (req, res) => {
  try {
    con.query(
      `SELECT p.id AS phone_id, p.title AS phone_name, c.name AS characteristic_name, cd.detail_name, cd.detail_info FROM  Products p JOIN  Characteristic_details cd ON p.id = cd.phone_id JOIN  Characteristics c ON c.id = cd.characteristic_id`,
      (err, result) => {
        if (!result || result.length === 0) {
          return res.status(404).send("No data");
        }
        ...
        return res.json(Object.values(phonesData));
      }
    );
  } catch (error) {
    if (error.code) {
  ...
  con.query("SELECT COUNT(*) as count FROM Categories", (err, result) => {
    ...
    for (let i = 1; i <= categories; i++) {
      app.get(`/categories/${i}/brands`, async (req, res) => {
        con.query(
          `SELECT DISTINCT (brand) FROM Products WHERE category = ${i}`,
          (err, result) => {
            return res.send(result);
          }
        );
